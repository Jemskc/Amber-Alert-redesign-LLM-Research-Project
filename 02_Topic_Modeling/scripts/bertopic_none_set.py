...
BERTopic - NONE SET
--------------------
Dataset : Topic-modeling-for-None-set-1.xlsx
          Comments where majority (2/3 or 3/3) of models labeled as NONE
Model   : all-mpnet-base-v2  (best quality, runs on RTX 4060 GPU)

Strategy (Grootendorst recommended):
  Step 1 → Find ALL fine-grained topics (MIN_TOPIC_SIZE=50)
  Step 2 → Intelligently merge similar topics down to 25
...
# CONFIG
# ─────────────────────────────────────────────
INPUT_FILE      = "Topic-modeling-for-None-set-1.xlsx"
DATASET_TAG     = "NONE_SET"
EMBED_MODEL     = "all-mpnet-base-v2"

MIN_TOPIC_SIZE  = 50    # fine-grained — finds all possible topics
N_GRAM_RANGE    = (1, 2)
TOP_N_WORDS     = 10
REDUCED_TOPICS  = 25    # target for paper-ready version
...
os.makedirs(os.path.join(output_dir, "full_138_topics",  "visualizations"), exist_ok=True)
os.makedirs(os.path.join(output_dir, "reduced_25_topics", "visualizations"), exist_ok=True)

print("=" * 60)
print("  BERTopic — AMBER ALERT NONE SET")
...
print("      (GPU accelerated — ~2-3 minutes)")
...
full_dir    = os.path.join(output_dir, "full_138_topics")
full_prefix = f"amber_alert_NONE_FULL_{timestamp}"
...
# ─────────────────────────────────────────────
# STEP 6 — REDUCE TO 25 TOPICS (paper version)
# ─────────────────────────────────────────────
...
reduced_dir    = os.path.join(output_dir, "reduced_25_topics")
reduced_prefix = f"amber_alert_NONE_REDUCED25_{timestamp}"
...
print("\n[7/7] Done!")
print("\n" + "=" * 60)
print("  COMPLETE — AMBER ALERT NONE SET")
print("=" * 60)
